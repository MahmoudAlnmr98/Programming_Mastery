...
class YouTubeDownloader:
    ...
    def start_download(self, entries):
        ...
        for entry in entries:
            ...
            # Handle Formats properly (yt-dlp needs specific instructions for audio vs video)
            ...
            try:
                with yt_dlp.YoutubeDL(options) as ydl:
                    ...
                    # Try to get real size, then approximate size, then default to 0
                    total_bytes = video_info.get('filesize') or video_info.get('filesize_approx') or 0
                    total_size += total_bytes

                    # Safely update UI from the background thread
                    size_mb = (total_bytes / (1024 * 1024)) if total_bytes > 0 else 0
                    self.master.after(0, self._update_treeview_finished, entry['title'], size_mb)

            except Exception as e:
                # Safely show error on main thread
                self.master.after(0, lambda err=e: messagebox.showerror("Error", f"Failed to download: {str(err)}"))

        total_size_mb = total_size / (1024 * 1024) if total_size > 0 else 0
        self.master.after(0, lambda: self.progress_label.config(text=f"All downloads completed! Total size: {total_size_mb:.2f} MB"))
    ...
    def create_progress_hook(self, entry):
        def progress_hook(d):
            if d['status'] == 'downloading':
                ...
                # Safely push updates to the main Tkinter thread
                self.master.after(0, self._update_ui_progress, entry['title'], progress_percent, total_size_mb, speed_mb)

        return progress_hook

    # --- NEW HELPER METHODS FOR THREAD-SAFE UI UPDATES ---
    def _update_ui_progress(self, title, percent, total_mb, speed_mb):
        self.progress_bar["value"] = percent
        for item in self.video_tree.get_children():
            if self.video_tree.item(item, "values")[0] == title:
                self.video_tree.item(item, values=(title, f"{total_mb:.2f} MB", "Downloading", f"{speed_mb:.2f} MB/s"))

    def _update_treeview_finished(self, title, size_mb):
        for item in self.video_tree.get_children():
            if self.video_tree.item(item, "values")[0] == title:
                self.video_tree.item(item, values=(title, f"{size_mb:.2f} MB", "Finished", "0.00 MB/s"))
